...
class LowLight_Dataset(data.Dataset):
    ...
    def load_pairs(self, folder_path):
        ...
        for idx, f_name in enumerate(low_list):
            pairs.append([
                 skimage.io.imread(os.path.join(folder_path, 'low', f_name)),  # [:, 4:-4, :],
                 skimage.io.imread(os.path.join(folder_path, 'high', f_name)),
                                            f_name.split('.')[0]])
    ...
    def __getitem__(self, item):
        lr = skimage.io.imread(f'{self.dark_root}/{self.dark_paths[item]}')
